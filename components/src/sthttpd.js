...
nsHttpServer.prototype =
{
  ...
  start: function(port)
  {
    ...
    var socket = new ServerSocket(this._port,
                                  false, // XXXsteve NOT loopback only
...
ServerHandler.prototype =
{
  ...
  handleResponse: function(connection, metadata)
  {
    ...
    try
    {
      try
      {
        // XXXskrul: Route all requests to the / handler
        if ("/" in this._overridePaths) {
          this._overridePaths["/"](metadata, response);
        }
        else {
...
const module =
{
  ...
  _objects:
  {
    server:
    {
      CID:         Components.ID("{01277be4-a9b8-4116-b9f2-e7f12c4ceb2d}"),
      contractID:  "@skrul.com/syrinxtape/jshttp;1",
